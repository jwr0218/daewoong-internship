...
def group_sort(df):
    ...
    find = df.groupby(df['EDI CODE'])
    find = find.mean()
    
    return find.sort_values(by = '계약단가(가)' ,ascending=False ,axis = 0 )




def EDI_Extract(df,edi):
    df['ediboo'] =df.apply(lambda x : EDIcode( x['EDI CODE'] ,edi), axis = 1 )
    
    return df[df['ediboo']==True].drop(['ediboo'],axis = 'columns')
...
def show(df,column):
    ...
    axes[0].set_title("이상치분석")
    
    axes[0].scatter(x = df[column], y = [0]*df.shape[0],alpha = 0.2,s = 300, c = df['label'])
    ...
    axes[1].set_title("tips box plot")
    plt.show()
